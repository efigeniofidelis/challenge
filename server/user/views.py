...
def user(request):
    ...
    url = f"http://api.etherscan.io/api?module=account&action=balance&address={wallet_address}&apikey={settings.ETHERSCAN_API_KEY}"
    res = requests.get(url=url)
    data = res.json()
    wallet_balance = data.get("result", 0)
